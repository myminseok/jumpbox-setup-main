...
echo "set service dhcp-server shared-network-name nsx-alb-mgmt-network subnet $NSX_ALB_MGMT_NETWORK_PREFIX.0/24 subnet-id 192168100"
echo "set service dhcp-server shared-network-name nsx-alb-mgmt-network subnet $NSX_ALB_MGMT_NETWORK_PREFIX.0/24 option default-router $NSX_ALB_MGMT_NETWORK_PREFIX.1"
echo "set service dhcp-server shared-network-name nsx-alb-mgmt-network subnet $NSX_ALB_MGMT_NETWORK_PREFIX.0/24 option name-server ${NAMESERVERS[0]}"
...
echo "set service dhcp-server shared-network-name tkg-mgmt-network subnet $TKG_MGMT_NETWORK_PREFIX.0/24 subnet-id 192168400"
echo "set service dhcp-server shared-network-name tkg-mgmt-network subnet $TKG_MGMT_NETWORK_PREFIX.0/24 option default-router $TKG_MGMT_NETWORK_PREFIX.1"
echo "set service dhcp-server shared-network-name tkg-mgmt-network subnet $TKG_MGMT_NETWORK_PREFIX.0/24 option name-server ${NAMESERVERS[0]}"
...
echo "set service dhcp-server shared-network-name tkg-workload-network subnet $TKG_WORKLOAD_NETWORK_PREFIX.0/24 subnet-id 192168460"
echo "set service dhcp-server shared-network-name tkg-workload-network subnet $TKG_WORKLOAD_NETWORK_PREFIX.0/24 option default-router $TKG_WORKLOAD_NETWORK_PREFIX.1"
echo "set service dhcp-server shared-network-name tkg-workload-network subnet $TKG_WORKLOAD_NETWORK_PREFIX.0/24 option name-server ${NAMESERVERS[0]}"
